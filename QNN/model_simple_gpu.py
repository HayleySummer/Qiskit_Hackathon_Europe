...
def select_action(state, env):
    ...
    if sample > eps_threshold:
        with torch.no_grad():
            ...
            print(policynet.shape)

            # Get a list of legal moves for the current player
            possibleMoves = env.state()[2]

            # Divide each move by 4 to get area of board
            possibleMovesSegment =  [m // segmentSize for m in possibleMoves]

            print(possibleMovesSegment)

            # Mask the probabilities returned by the network to only include legal moves
            possiblepolicy = torch.tensor([policynet[0][i] for i in possibleMovesSegment])
            policynet = np.array([policynet.data.cpu().numpy().flatten()])[0]
            policynet = [policynet[i] if i in possibleMovesSegment else 0 for i in range(int(64/segmentSize))]
            ...
            if math.isnan(policynet[0]):
                ...
                policymax = np.random.choice(int(64/segmentSize), p=policynet)
            
            # Ensure the move is legal and return it
            if policymax in possibleMovesSegment:
