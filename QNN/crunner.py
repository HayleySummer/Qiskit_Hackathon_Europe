...
class Move:
    ...
    def make_move(self, move):
        ...
        #change segment to move
        move = self.segment_move(move)
    ...
    def make_move_segment(self, move):
        ''' Make the specified move and then run the c program to make an opposing move and update the board '''
        proc = subprocess.Popen(["./compiledothello.c"],
        stdin=subprocess.PIPE, stdout=subprocess.PIPE)
        #change segment to move
	...
	# Calculate the segment 0 - 15
        segment = move * 16
        #find moves in range segment to segment + 4

        print("Self.Moves:", self.moves, " Segment: ", segment/16)
        
        segmentMoves = []
        for i in self.moves:
            if ((i >= segment) and (i < (segment + 16))):
                segmentMoves.append(i)

        print("SegmentMoves=", segmentMoves)
        
        
        if len(segmentMoves) > 1:
            moveToMake = segmentMoves[random.randint(0,len(segmentMoves)-1)]
        else:
            moveToMake = segmentMoves[0]
    ...
    def state(self):
        ...
        "Make sure only 16 possible moves are returned"
        #if self.moves 
